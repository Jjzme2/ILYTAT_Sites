...
useHead({ title: 'Create Account — ILYTAT Sites' })
...
const form = reactive({ name: '', email: '', password: '', businessName: '' })
...
const handleSubmit = async () => {
  ...
  try {
    ...
    toast.add({ title: msg.includes('email-already-in-use') ? 'An account already exists with that email' : 'Registration failed', color: 'error' })
  } finally {
    loading.value = false
  }
}
  ...
  <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6">
    <div class="w-full max-w-md">
      <div class="text-center mb-10">
        ...
        <h1 class="font-display text-3xl font-bold text-white">Create an account</h1>
        <p class="text-slate-400 mt-2">Track your orders and download deliverables</p>
      </div>
      ...
      <div class="glass border border-white/5 rounded-2xl p-8">
        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="text-sm text-slate-300 font-medium mb-2 block">Full Name</label>
              <input
                v-model="form.name"
                type="text"
                required
        ...
        <p class="text-center text-sm text-slate-400 mt-6">
          ...
          <NuxtLink to="/auth/login" class="text-orange-400 hover:text-orange-300 font-medium ml-1">
            Sign in
          </NuxtLink>
        </p>
      </div>
